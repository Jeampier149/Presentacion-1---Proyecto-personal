...
use App\Models\Legajo\CompensacionModel;
use Illuminate\Http\Request;
use Illuminate\Http\JsonResponse;
use Illuminate\Support\Facades\Storage;
use Illuminate\Support\Facades\Validator;
use stdClass;
class CompensacionController extends JSONResponseController{
    public function __construct()
    {
        $this->middleware('auth:sanctum');
    }
    public function listarCompensaciones(Request $request): JsonResponse
    {
        ...
        $compensacionModel = new CompensacionModel();
        ...
        $resultado = $compensacionModel->listarCompensaciones($documento,$tipo,$asunto,$fecha,$dni,$pagina,$longitud);
        
        return $this->sendResponse(200, true, '', $resultado);
    }

    public function registrarCompensacion(Request $request): JsonResponse{
       [$usuario, $perfil, $equipo] = $this->getHost($request);
        ...
        $cmp= new CompensacionModel();
        $resultado=$cmp->registrarCompensacion($datos,$usuario,$equipo,$perfil);
        return $this->sendResponse(200, true, '', $resultado);
    }
    public function getCompensacion(Request $request){
         $id=$request->post('id');

         $cm=new CompensacionModel();
         $resultado=$cm->getCompensacion($id);
         return $this->sendResponse(200, true, '', $resultado);
    }
    public function editarCompensacion(Request $request): JsonResponse{
        [$usuario, $perfil, $equipo] = $this->getHost($request);
         $datos = json_decode($request->post('datos'), true);
         $numeroDocumento = json_decode($request->post('numeroDoc'), true);
         if ($request->hasFile('archivo')) {
         ...
         $cmp= new CompensacionModel();
         $resultado=$cmp->editarCompensacion($datos,$usuario,$equipo,$perfil);
         return $this->sendResponse(200, true, '', $resultado);
     }
